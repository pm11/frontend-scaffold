...
export const Todos = {
  ...
  get: (id: string) => resolveWithDelay(todosResponse.find(t => t.id === id)),
  create: (payload: ITodoModel) =>
    resolveWithDelay(todosResponse.push(payload)),
  update: (payload: ITodoModel) =>
    resolveWithDelay(
      todosResponse.map(t => (t.id === payload.id ? payload : t))
    ),
  delete: (id: string) =>
    resolveWithDelay(todosResponse.filter(t => t.id !== id))
}
